...
# and write the INDELs into mg-ref-output/INDEL.extract.chrxx.data, where xx is a chromosome identified from the reference genome.
 ...
 docker run --rm -it -v "$(pwd)/input:/input" -v "$(pwd)/ref-output:/mg-ref-output" bwbble-ref:dev data_prep /input/$VCF
...
# ref_w_snp_and_bubble.fasta combines the reference genome with the SNPs and the INDELs.
 ...
 docker run --rm -it -v "$(pwd)/input:/input" -v "$(pwd)/ref-output:/mg-ref-output" bwbble-ref:dev comb /input/$HUMAN_GENOME /mg-ref-output/ref_w_snp.fasta /mg-ref-output/ref_w_snp_and_bubble.fasta /mg-ref-output/bubble.data 
...
# Now we're doing an alignment job
 ...
 docker run --rm -it -v "$(pwd)/input:/input" -v "$(pwd)/ref-output:/mg-ref-output" -v "$(pwd)/align-output:/mg-align-output" bwbble-align:dev align /mg-ref-output/ref_w_snp.fasta /input/$READS /mg-align-output/aligned_reads.aln
