...
# and write the INDELs into mg-ref-output/INDEL.extract.chrxx.data, where xx is a chromosome identified from the reference genome.
 ...
 docker run --rm -it -v "$(pwd)/input:/input" -v "$(pwd)/ref-output:/mg-ref-output" bwbble-ref:dev data_prep /input/ALL.chr21.phase1_release_v3.20101123.snps_indels_svs.genotypes.vcf 
...
# ref_w_snp_and_bubble.fasta combines the reference genome with the SNPs and the INDELs.
 ...
 docker run --rm -it -v "$(pwd)/input:/input" -v "$(pwd)/ref-output:/mg-ref-output" bwbble-ref:dev comb /input/human_g1k_v37.fasta /mg-ref-output/ref_w_snp_chr21.fasta /mg-ref-output/ref_w_snp_chr21_and_bubble.fasta /mg-ref-output/bubble21.data 
...
# Now we're doing an alignment job
 ...
 docker run --rm -it -v "$(pwd)/input:/input" -v "$(pwd)/ref-output:/mg-ref-output" -v "$(pwd)/align-output:/mg-align-output" bwbble-align:dev align /mg-ref-output/ref_w_snp_chr21_and_bubble.fasta /input/sim_chr21_N100.fastq /mg-align-output/sim_chr21_N100.aln

 docker run --rm -it -v "$(pwd)/input:/input" -v "$(pwd)/ref-output:/mg-ref-output" -v "$(pwd)/align-output:/mg-align-output" bwbble-align:dev aln2sam /mg-ref-output/ref_w_snp_chr21_and_bubble.fasta /input/sim_chr21_N100.fastq /mg-align-output/sim_chr21_N100.aln /mg-align-output/sim_chr21_N100.sam
